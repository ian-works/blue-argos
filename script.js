...
(function () {
  ...
  var I18N = {
    ko: {
      ...
      "hero.lead": "BlueArgos는 불안하게 개인 정보나 차량 정보를 요구하지 않습니다. 경로 설정도 필요 없습니다. 설치 후 실행만 하면 됩니다. 실행 중 데이터를 전혀 사용하지 않고 최소한의 배터리로 동작합니다. 그러면서도 <strong>BlueArgos만의 기술로 편리함을 제공</strong>합니다.",
    ...
    en: {
      ...
      "hero.lead": "BlueArgos never makes you uneasy by asking for personal or vehicle information. You don't need to set a route, either — just install it and run it. While it's running, it uses no data at all and runs on minimal battery. Even so, <strong>BlueArgos's own technology still keeps it convenient</strong>.",
